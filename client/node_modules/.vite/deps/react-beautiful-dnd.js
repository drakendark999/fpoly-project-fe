import {
  ...
  offset,
  withScroll
} from "./chunk-4H6UGYQV.js";
import "./chunk-DMHJNZZW.js";
import {
  _extends,
...
import {
  ...
  compose,
  createStore
} from "./chunk-QTXZELWA.js";
import "./chunk-Q3M5E7PX.js";
...
var getClientFromPageBorderBoxCenter = function(_ref) {
  ...
  var offset2 = subtract(withoutPageScrollChange, draggable2.page.borderBox.center);
...
var update = function(_ref) {
  ...
  var offset2 = subtract(clientSelection, state.initial.client.selection);
  var client = {
    offset: offset2,
...
var moveTo = function moveTo2(offset2) {
  return isEqual2(offset2, origin) ? null : "translate(" + offset2.x + "px, " + offset2.y + "px)";
};
var transforms = {
  moveTo,
  drop: function drop4(offset2, isCombining) {
    var translate = moveTo(offset2);
    if (!translate) {
      return null;
    }
    if (!isCombining) {
...
var getNewHomeClientOffset = function(_ref) {
  ...
  var offset2 = subtract(newClientCenter, draggable2.client.borderBox.center);
  return offset2;
};
...
function getScrollListener(_ref) {
  ...
  function start3() {
    !!isActive() ? true ? invariant(false, "Cannot start scroll listener when already active") : invariant(false) : void 0;
    unbind = bindEvents(window, [binding]);
  }
  function stop() {
    !isActive() ? true ? invariant(false, "Cannot stop scroll listener when not active") : invariant(false) : void 0;
    scheduled.cancel();
    unbind();
    unbind = noop;
  }
...
var createJumpScroller = function(_ref) {
  ...
  var moveByOffset = function moveByOffset2(state, offset2) {
    var client = add(state.current.client.selection, offset2);
    move3({
      client
    });
  };
...
function tryStart(_ref3) {
  ...
  function lift$12(args) {
    ...
    if (phase !== "PRE_DRAG") {
      completed();
      !(phase === "PRE_DRAG") ? true ? invariant(false, "Cannot lift in phase " + phase) : invariant(false) : void 0;
    }
...
function getDraggingStyle(dragging) {
  ...
  var offset2 = dragging.offset, combineWith = dragging.combineWith, dropping = dragging.dropping;
  var isCombining = Boolean(combineWith);
  var shouldAnimate = getShouldDraggingAnimate(dragging);
  var isDropAnimating = Boolean(dropping);
  var transform = isDropAnimating ? transforms.drop(offset2, isCombining) : transforms.moveTo(offset2);
  var style2 = {
    position: "fixed",
...
function useValidation$1(props, contextId, getRef) {
  useDevSetupWarning(function() {
    ...
    !isInteger(props.index) ? true ? invariant(false, prefix2(id) + " requires an integer index prop") : invariant(false) : void 0;
...
function getDraggableSelector() {
  ...
  var getMemoizedProps = memoize_one_esm_default(function(offset2, mode, dimension, isClone, draggingOver, combineWith, forceShouldAnimate) {
    return {
      mapped: {
        type: "DRAGGING",
        dropping: null,
  ...
  var selector = function selector2(state, ownProps) {
    if (state.isDragging) {
      ...
      var offset2 = state.current.client.offset;
...
function getSecondarySelector() {
  ...
  var getMemoizedProps = memoize_one_esm_default(function(offset2, combineTargetFor, shouldAnimateDisplacement) {
  ...
  var getProps = function getProps2(ownId, draggingId, impact, afterCritical) {
    ...
    var offset2 = memoizedOffset(displaceBy.x, displaceBy.y);
